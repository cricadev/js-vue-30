...
async function generateMarkDown(commits, config) {
  ...
  await Promise.all(Array.from(_authors.keys()).map(async (authorName) => {
    ...
    for (const email of meta.email) {
      const { user } = await fetch(`https://ungh.unjs.io/user/find/${email}`).then((r) => r.json()).catch(() => ({ user: null }));
      if (user) {
...
async function bumpVersion(commits, config) {
  ...
  const pkg = JSON.parse(await promises.readFile(pkgPath, "utf8").catch(() => "{}")) || {};
...
export { getCurrentGitBranch as a, getCurrentGitTag as b, getCurrentGitRef as c, getGitDiff as d, parseGitCommit as e, generateMarkDown as f, getLastGitTag as g, determineSemverChange as h, bumpVersion as i, loadChangelogConfig as l, parseCommits as p };
